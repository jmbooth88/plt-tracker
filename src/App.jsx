  ...
  if (split === "ppl") {
    const maps = {
      3: [
        ...
        { id: "fri", label: "Friday",    focus: "Legs", env: hasOutdoor ? "Outdoor" : "Gym" },
      ],
      4: [
        { id: "mon", label: "Monday",    focus: "Push",        env: "Gym" },
...
function applyUpperLimitation(exercises, type) {
  return exercises.map(e => {
    if (e.category !== "push") return e;
    if (type === "upper_pain") return { ...e, name: "Push-Up", sets: Math.min(e.sets, 3), reps: "10–15", rpe: 6, note: "Pain-free range only. Stop at any discomfort.", pr: null };
    if (type === "upper_cautious") {
...
function applyLowerLimitation(exercises, type) {
  return exercises.map(e => {
    if (type === "lower_pain") {
      if (e.category === "squat") return { ...e, name: "Seated Leg Press", sets: Math.min(e.sets, 3), rpe: 6, note: "Pain-free range. No deep flexion if uncomfortable." };
      if (e.category === "hinge" && e.name.includes("Deadlift")) return { ...e, name: "Glute Bridge", sets: Math.min(e.sets, 3), reps: "15", rpe: 6, note: "Hinge pattern without spinal load.", pr: null };
    }
    if (type === "lower_cautious") {
      if (e.name === "Bulgarian Split Squat") return { ...e, name: "Step-Up", note: "Controlled. Drive through heel. Sub for split squat." };
      if (e.rpe >= 8 && e.category === "hinge") return { ...e, rpe: 7, sets: Math.max(e.sets - 1, 2), note: (e.note || "") + " Reduced — lower body caution." };
    }
    return e;
  }).filter(Boolean);
}
...
function buildProgram(answers) {
  ...
  if (split === "upper_lower") {
    const lowerStrength = [
      { name: hasBarbell ? "Trap Bar Deadlift" : "Romanian Deadlift", sets: volMod(4), reps: "4–5", rpe: 8, note: "Neutral spine. Reset between reps.", category: "hinge", ...(dlPR ? { pr: +dlPR } : {}) },
    ...
    const upperPull = [
      ...
      { name: "Neutral-Grip Pull-Up",    sets: volMod(3), reps: "8–10",  rpe: 7, note: "Full hang at bottom.", category: "pull" },
      ...
      { name: "Band External Rotation",  sets: 2, reps: "15–20", rpe: null, note: "Per side.", category: "rehab" },
      { name: "Bicep Curl",              sets: 2, reps: "12",   rpe: 6, note: "Neutral or supinated grip.", category: "accessory" },
    ];
    const lowerStamina = hasOutdoor ? [
      { name: "Weighted Ruck",         sets: 1, reps: "35min", rpe: null, note: "30–40 lbs. Nasal breathing only.", category: "cardio" },
      { name: "Bulgarian Split Squat", sets: volMod(3), reps: "8", rpe: 7, note: "Per leg. Post-ruck.", category: "squat" },
      { name: "Kettlebell Swing",      sets: volMod(4), reps: "15", rpe: 7, note: "Hip drive, not squat.", category: "hinge" },
    ] : [
      { name: hasBarbell ? "High Bar Back Squat" : "Goblet Squat", sets: volMod(4), reps: "5", rpe: 7, note: "Upright torso.", category: "squat" },
    ...
    const upperPush = [
      equipment === "bodyweight"
        ? { name: "Push-Up", sets: volMod(4), reps: "15–20", rpe: 7, note: "Controlled eccentric.", category: "push" }
        : { name: "Landmine Press", sets: volMod(4), reps: "10", rpe: 7, note: "Shoulder-friendly arc.", category: "push" },
      ...
      { name: "Tricep Overhead Ext.",   sets: 2, reps: "15",  rpe: 6, note: "Long head.", category: "accessory" },
      { name: "Hammer Curl",            sets: 2, reps: "12",  rpe: 6, note: "Brachialis focus.", category: "accessory" },
      { name: "Face Pull",              sets: 2, reps: "15",  rpe: 6, note: "Shoulder insurance.", category: "rehab" },
    ];
    const cycle = [lowerStrength, upperPull, lowerStamina, upperPush];
    days.forEach((d, i) => { prog[d.id] = cycle[i % cycle.length]; });
  }

  else if (split === "ppl") {
    const push = [
      { name: equipment === "bodyweight" ? "Push-Up" : hasBarbell ? "Bench Press" : "Incline DB Press", sets: volMod(4), reps: hasBarbell ? "5" : "10", rpe: 8, note: "Primary horizontal push.", category: "push" },
      { name: "Landmine Press",         sets: volMod(3), reps: "10", rpe: 7, note: "Shoulder-friendly arc.", category: "push" },
      { name: equipment === "bodyweight" ? "Pike Push-Up" : "DB Shoulder Press", sets: volMod(3), reps: "12", rpe: 7, note: "Vertical push.", category: "push" },
      { name: "Lateral Raise",          sets: 3, reps: "15", rpe: 6, note: "Light. Medial delt.", category: "accessory" },
      { name: "Tricep Overhead Ext.",   sets: 3, reps: "15", rpe: 6, note: "Long head stretch.", category: "accessory" },
      { name: "Band Pull-Apart",        sets: 3, reps: "20", rpe: null, note: "Every press session.", category: "rehab" },
    ];
    const pull = [
      { name: "Neutral-Grip Pull-Up",   sets: volMod(4), reps: "8–10",  rpe: 7, note: "Full hang.", category: "pull" },
      ...
      { name: "Hammer Curl",            sets: 2, reps: "12", rpe: 6, note: "Brachialis.", category: "accessory" },
    ];
    const legs = [
      { name: hasBarbell ? "Back Squat" : "Goblet Squat", sets: volMod(4), reps: hasBarbell ? "5" : "8", rpe: 8, note: "Primary squat.", category: "squat", ...(dlPR ? { pr: +dlPR } : {}) },
      { name: hasBarbell ? "Romanian Deadlift" : "Single-Leg RDL", sets: volMod(3), reps: "8–10", rpe: 7, note: "3s eccentric.", category: "hinge" },
      ...
      { name: "Dead Bug", sets: 2, reps: "8", rpe: null, note: "Per side.", category: "core" },
    ];
    days.forEach((d, i) => { prog[d.id] = [push, pull, legs][i % 3]; });
  }

  else if (split === "fullbody") {
    const fbA = [
      { name: hasBarbell ? "Trap Bar Deadlift" : "Romanian Deadlift", sets: volMod(4), reps: "5", rpe: 8, note: "Primary hinge.", category: "hinge", ...(dlPR ? { pr: +dlPR } : {}) },
    ...
    days.forEach((d, i) => { prog[d.id] = [fbA, fbB, conditioning, fbA][i % 4]; });
  }
    ...
    const chest = [
      ...
      { name: equipment === "bodyweight" ? "Push-Up" : "Lateral Raise", sets: volMod(3), reps: "15", rpe: 6, note: "Accessory work.", category: equipment === "bodyweight" ? "push" : "accessory" },
    ...
    const legs = [
      { name: hasBarbell ? "Back Squat" : "Goblet Squat", sets: volMod(4), reps: hasBarbell ? "5" : "8", rpe: 8, note: "Primary squat.", category: "squat" },
      { name: hasBarbell ? "Romanian Deadlift" : "Single-Leg RDL", sets: volMod(3), reps: "8–10", rpe: 7, note: "Hip hinge.", category: "hinge" },
      { name: "Bulgarian Split Squat",  sets: volMod(3), reps: "8", rpe: 7, note: "Per leg.", category: "squat" },
      { name: hasOutdoor ? "Weighted Ruck" : hasBarbell ? "Trap Bar Farmer's Carry" : "Suitcase Carry", sets: hasOutdoor ? 1 : 3, reps: hasOutdoor ? "30min" : "40yd", rpe: null, note: hasOutdoor ? "Nasal breathing." : "Grip and posture.", category: hasOutdoor ? "cardio" : "carry" },
      { name: "Nordic Curl",            sets: 2, reps: "5", rpe: 8, note: "Eccentric only if needed.", category: "accessory" },
    ];
    const shoulders = [
    ...
    days.forEach((d, i) => { prog[d.id] = [chest, back, legs, shoulders][i % 4]; });
  }

  // Limitation modifications
  if (limitation === "upper_pain" || limitation === "upper_cautious") {
    Object.keys(prog).forEach(id => { prog[id] = applyUpperLimitation(prog[id], limitation); });
  }
  if (limitation === "lower_pain" || limitation === "lower_cautious") {
    Object.keys(prog).forEach(id => { prog[id] = applyLowerLimitation(prog[id], limitation); });
  }
  if (limitation === "both_pain") {
    Object.keys(prog).forEach(id => {
      prog[id] = applyUpperLimitation(applyLowerLimitation(prog[id], "lower_pain"), "upper_pain");
    });
  }
  ...
  if (primaryGoal === "strength") {
    Object.keys(prog).forEach(id => {
      prog[id] = prog[id].map(e => (e.category === "hinge" || e.category === "squat") && e.rpe >= 7 ? { ...e, sets: Math.min(e.sets + 1, 5) } : e);
    });
  }
  if (primaryGoal === "stamina") {
    Object.keys(prog).forEach(id => {
      prog[id] = prog[id].map(e => (e.category === "carry" || e.category === "cardio") ? { ...e, sets: Math.min((e.sets || 1) + 1, 5) } : e);
    });
  }
...
function OnboardingWizard({ onComplete }) {
  ...
  const [answers, setAnswers] = useState({
    name: "", primaryGoal: "longevity", split: "upper_lower", dayCount: 4,
    environment: "mix", equipment: "full", limitation: "none",
    layoffWeeks: 6, dlPR: "", squatPR: "", bodyweight: "",
  });
    ...
    switch (stepName) {
      case "welcome": return (
        <div>
          ...
          <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.7, marginBottom: 28 }}>A few questions to generate a Week 1 training plan built around your goals, schedule, equipment, and any current limitations. About 90 seconds.</div>
          <div style={{ borderTop: `1px solid ${C.faint}`, paddingTop: 20, marginBottom: 24 }}>
            {[["Program split tailored to your goals", C.accent], ["Equipment-aware exercise selection", C.blue], ["Limitation-safe modifications built in", C.green], ["Indoor, outdoor, or mixed sessions", "#A07EC8"]].map(([text, col]) => (
      ...
      case "split": return (
        <div>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 22, lineHeight: 1.6 }}>How do you want to organize training across the week?</div>
          {[
            { val: "upper_lower", label: "Upper / Lower Split",   desc: "Alternate upper and lower days. Best balance of frequency and recovery.", color: C.accent },
            { val: "ppl",         label: "Push / Pull / Legs",    desc: "Dedicated push, pull, and leg days. Great for hypertrophy and strength.", color: C.blue },
            { val: "fullbody",    label: "Full Body",              desc: "Hit all major patterns every session. Best for 3-day schedules.", color: C.green },
            { val: "conventional",label: "Conventional Split",    desc: "Classic chest/back/legs/shoulders. Familiar structure, dedicated muscle days.", color: "#A07EC8" },
          ].map(({ val, label, desc, color }) => (
      ...
      case "days": return (
        <div>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 22, lineHeight: 1.6 }}>How many days can you realistically commit to every week? Recovery is part of the program.</div>
          {[
            { val: 3, label: "3 days", desc: "Solid foundation. Full recovery between sessions." },
            { val: 4, label: "4 days", desc: "Optimal for most. Enough frequency without outrunning recovery." },
            { val: 5, label: "5 days", desc: "Higher volume. Requires solid sleep and nutrition." },
            { val: 6, label: "6 days", desc: "PPL double-run. Only if recovery is dialed in." },
          ].filter(({ val }) => {
      ...
      case "environment": return (
        <div>
          ...
          {[
            ...
            { val: "outdoor", label: "Outdoor only", desc: "Trail work, rucks, and carries outdoors. Minimal equipment.", color: C.green },
      ...
      case "equipment": return (
        <div>
          ...
          {[
            { val: "full",        label: "Full commercial gym",   desc: "Barbells, rack, cables, machines, dumbbells — everything.", color: C.accent },
            { val: "barbell",     label: "Barbell + rack + DBs",  desc: "Home gym setup. Full strength work, no cables or machines.", color: C.blue },
            { val: "dumbbells",   label: "Dumbbells only",        desc: "No barbell. DB alternatives used throughout.", color: C.green },
            { val: "kettlebells", label: "Kettlebells",           desc: "KB-primary. Swings, cleans, carries, presses.", color: "#A07EC8" },
            { val: "bodyweight",  label: "Bodyweight only",       desc: "No equipment. Push-up, pull-up, squat, carry progressions.", color: "#C87E96" },
          ].map(({ val, label, desc, color }) => (
      ...
      case "limitation": return (
        <div>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 22, lineHeight: 1.6 }}>The program modifies affected movements. You can update this anytime via the AI tab.</div>
          {[
            { val: "none",           label: "None — train fully",                   desc: "No restrictions.", color: C.green },
            { val: "upper_cautious", label: "Upper body — pain-free but cautious",  desc: "Shoulder, elbow, or wrist history. Conservative pressing. Landmine entry.", color: C.accent },
            { val: "upper_pain",     label: "Upper body — currently uncomfortable", desc: "Active upper pain. Pressing replaced with push-up/band work only.", color: C.red },
            ...
            { val: "lower_pain",     label: "Lower body — currently uncomfortable", desc: "Active lower pain. Heavy squats and hinges replaced with low-load alternatives.", color: C.red },
            { val: "both_pain",      label: "Both upper and lower",                 desc: "Multiple active restrictions. Conservative across all patterns.", color: "#A07EC8" },
          ].map(({ val, label, desc, color }) => (
          ...
          <div style={{ marginTop: 16 }}>
            <div style={{ fontSize: 9, color: C.muted, letterSpacing: 1, marginBottom: 8, ...mono }}>TRAINING LAYOFF</div>
            {[{ val: 0, label: "No layoff" }, { val: 3, label: "1–4 weeks off" }, { val: 6, label: "4–8 weeks off" }, { val: 10, label: "8+ weeks off" }].map(({ val, label }) => (
              <div key={val} onClick={() => update("layoffWeeks", val)} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, cursor: "pointer", padding: "8px 12px", background: answers.layoffWeeks === val ? `${C.accent}12` : C.card, border: `1px solid ${answers.layoffWeeks === val ? C.accent : C.border}`, borderRadius: 5 }}>
      ...
      case "baselines": return (
        <div>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 22, lineHeight: 1.6 }}>Used to suggest working weights and track progress. Skip any you don't know.</div>
          {[
      ...
      case "summary": {
        const { program: prev, days: previewDays } = buildProgram(answers);
        ...
        const limLabels   = { none: "None", upper_cautious: "Upper — cautious", upper_pain: "Upper — pain", lower_cautious: "Lower — cautious", lower_pain: "Lower — pain", both_pain: "Upper + lower" };
        return (
          <div>
            <div style={{ fontSize: 9, color: C.green, letterSpacing: 3, marginBottom: 10, ...mono }}>PROGRAM READY</div>
            <div style={{ ...serif, fontSize: 26, color: C.text, marginBottom: 6 }}>{answers.name ? `Here's your program, ${answers.name}.` : "Here's your Week 1 program."}</div>
            ...
            <div style={{ background: "#0D1A0D", border: `1px solid #1E3A1E`, borderRadius: 6, padding: "12px 16px", marginBottom: 20 }}>
              ...
              {previewDays.map(d => (
                <div key={d.id} style={{ display: "flex", gap: 12, marginBottom: 8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>{(prev[0]?.[d.id] ?? []).slice(0, 3).map(e => e.name).join(" · ")}</div>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text }}>
      ...
      <div style={{ padding: "20px 24px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {step > 0 && step < ONBOARDING_STEPS.length - 1 && <div style={{ fontSize: 9, color: C.muted, letterSpacing: 1, ...mono }}>{step} / {totalSteps}</div>}
      </div>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: C.bg, borderTop: `1px solid ${C.border}`, padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        {step > 0 ? (
          <button onClick={() => setStep(s => Math.max(s - 1, 0))} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 5, color: C.muted, padding: "10px 20px", cursor: "pointer", fontSize: 10, fontWeight: 700, letterSpacing: 1, ...mono }}>← BACK</button>
        ) : <div />}
        {isLast ? (
          <button onClick={() => { const { program: p, days: d } = buildProgram(answers); onComplete(p, d, answers); }} style={{ background: C.green, border: "none", borderRadius: 5, color: "#000", padding: "12px 32px", cursor: "pointer", fontSize: 11, fontWeight: 700, letterSpacing: 2, ...mono }}>LOAD MY PROGRAM →</button>
        ) : (
          <button onClick={() => setStep(s => Math.min(s + 1, ONBOARDING_STEPS.length - 1))} style={{ background: C.accent, border: "none", borderRadius: 5, color: "#000", padding: "12px 28px", cursor: "pointer", fontSize: 11, fontWeight: 700, letterSpacing: 2, ...mono }}>{step === 0 ? "GET STARTED →" : "NEXT →"}</button>
        )}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const allPRs = {};
  Object.entries(log).forEach(([wi, dayMap]) => Object.entries(dayMap).forEach(([dayId, exs]) => Object.entries(exs).forEach(([name, data]) => {
